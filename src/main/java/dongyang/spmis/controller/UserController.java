...
import dongyang.spmis.service.DocumentService;
...
//	}
	...
	public void checknumconfirm(Model model, UserDTO user
		...
		res.setContentType("text/html; charset=UTF-8");
		PrintWriter out = res.getWriter();
		log.info("매칭 확인 checknum=\" + user.getChecknum()");
...
//	}
	...
	public String mypage(Model model, HttpServletRequest req, HttpServletResponse res) {
		...
		model.addAttribute("projectlist", documentService.select(userInfo));
	...
	public String mypageUpdateProfileImg(Model model,  HttpServletRequest req, HttpServletResponse res,
			@RequestParam("upload-img") MultipartFile profile_img, Authentication authentication) throws IOException {

		UserDTO userInfo = (UserDTO) session.getAttribute("mem");

		String profile_img_url =  userService.updateProfileImg(authentication.getName(), profile_img);
		// 세션을 갱신하여 프로필 변경사항을 웹 뷰에서 즉시 적용
//        userService.updateProfileImg(profile_img_url, userInfo.getEmail());
		userInfo.setProfile(profile_img_url);
