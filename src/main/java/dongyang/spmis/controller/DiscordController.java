...
public class DiscordController {
    ...
    public void addWebhookLink(WebhookLinkDTO webhook, HttpServletResponse res) throws IOException {
        log.info(webhook.getWebhook_link());
        System.out.println(webhook.getWebhook_link());
        res.setContentType("text/html; charset=UTF-8");
        PrintWriter out = res.getWriter();
        WHProperties result = webHookService.addWebhookLink(webhook.getProject_id(), webhook.getWebhook_link());
        switch(result){
    ...
    public void sendMessage(ProjectNoticeDTO notice, HttpServletResponse res) throws IOException {
        res.setContentType("text/html; charset=UTF-8");
        PrintWriter out = res.getWriter();
        WHProperties result = webHookService.callEvent(notice.getProject_id(), notice.getNotice());
        switch(result){
            case FIND_NOT_LINK:
                out.println("<script>");
