...
import dongyang.spmis.model.Task.TaskJoinUserDTO;
...
public class ProjectController {
	...
	public String projects(Model model, HttpServletRequest req, HttpServletResponse res) {
		...
		ArrayList<ProjectListDTO> projectList = projectService.selectProjectList(userInfo.getUser_email());
	...
	@GetMapping("task_data_update")
	@ResponseBody
	public String taskEdit(int task_id, Model model){
		System.out.println("task_data_update");
		model.addAttribute("projectTask", projectService.findProjectTask(task_id));
		return "projectTask";
	}

	@GetMapping("task_comment")
	public String taskComment(int task_id, Model model){
		System.out.println("task_comment");
		ArrayList<TaskCommentDTO> comments = projectService.selectComment(task_id);
		if(comments.isEmpty()){
...
//			projectService.deleteProjectRule(project);
	...
	public String settings(Model model, ProjectDTO project, HttpServletRequest req, HttpServletResponse res) 
		...
		for (ProjectNoticeDTO notice : notices) {
...
//        task.setFinal_date(formatter.parse(finaltime));
	...
	public void updateKanbanStatus(Model model, UpdateKanbanDTO kanban, String starTime, String finalTime, HttpServletRequest req, HttpServletResponse res)
			throws IOException, ParseException {
		res.setContentType("text/html; charset=UTF-8");
    	PrintWriter out = res.getWriter();
		ProjectDTO project = projectService.selectOneProject(kanban.getProject_id());
		System.out.println("kanban.getKanban_id() = " + kanban.getKanban_id());
		if(selectGroupCheckInProject(project, req, res)) {
			
			if(projectService.updateTaskKanbanID(kanban)) {
				out.println("<script>");
...
//    	
	...
	public void commentPOST(TaskCommentDTO comment, int project_id, HttpServletResponse res, HttpServletRequest req ) throws IOException {
		session = req.getSession();
		UserDTO userInfo = (UserDTO) session.getAttribute("mem");
	...
	@PostMapping("projectFinal")
	public void updateProjectFinalDate(int project_id, HttpServletResponse res) throws IOException {
		res.setContentType("text/html; charset=UTF-8");
		PrintWriter out = res.getWriter();
		PSProperties result = projectService.updateProjectFinal(project_id);
		switch (result){
			case SUCCESS:
				out.println("<script>");
				out.println("alert('프로젝트 종료');");
				out.println("location.href='/settings?project_id=" + project_id + "';");
				out.println("</script>");
				break;
			case FAILED:
				out.println("<script>");
				out.println("alert('프로젝트 종료 실패');");
				out.println("location.href='/settings?project_id=" + project_id + "';");
				out.println("</script>");
				break;
		}
	...
	@PostMapping("cancelProjectFinalDate")
	public void cancelProjectFinalDate(int project_id, HttpServletResponse res) throws IOException {
		res.setContentType("text/html; charset=UTF-8");
		PrintWriter out = res.getWriter();
		PSProperties result = projectService.cancelProjectFinal(project_id);
		switch (result){
			case SUCCESS:
				out.println("<script>");
				out.println("alert('프로젝트 종료');");
	...
	@PostMapping("updateProjectFinalExpectDate")
	public void updateProjectFinalDate(String project_final_expect_date, int project_id , HttpServletResponse res)
			throws IOException, ParseException {
		SimpleDateFormat formatter = new SimpleDateFormat("yyyy-MM-dd'T'HH:mm");
		res.setContentType("text/html; charset=UTF-8");
		PrintWriter out = res.getWriter();
		PSProperties result = projectService.updateProjectExpectFinal(
				new ProjectFinalExpectDateDTO(project_id, formatter.parse(project_final_expect_date)));
		switch(result){
			case SUCCESS :
				out.println("<script>");
				out.println("alert('프로젝트 예상 종료일 수정 성공');");
				out.println("location.href='/settings?project_id=" + project_id + "';");
				out.println("</script>");
				break;
			case NO_PERMISSION:
				out.println("<script>");
				out.println("alert('수정 자격이 없습니다.');");
				out.println("location.href='/settings?project_id=" + project_id + "';");
				out.println("</script>");
				break;
			case FAST_THAN_START_TIME:
				out.println("<script>");
				out.println("alert('예상 종료시간이 현재 프로젝트 생성시간보다 빠릅니다.');");
				out.println("location.href='/settings?project_id=" + project_id + "';");
				out.println("</script>");
				break;
			case FAILED:
