...
public interface UserMapper extends DefaultDBInfo {
	...
	String findUserNameByUserEmail(String user_email);
	...
	// 회원 프로필 이미지 변경 메서드
	@Update("UPDATE " + USER + " SET profile=#{img_url} where user_email=#{user_email}")
	boolean updateProfileImg( @Param("img_url") String img_url, @Param("user_email") String user_email);
}
